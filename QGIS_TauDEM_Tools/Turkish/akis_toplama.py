...
Contributing Area aracını kullanarak akış toplamayı hesaplamaktadır.
...
class FlowAccum(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        fa_dem_abs_path = os.path.join(parameters['output_dir'], 'fa_'+fd_name)
        
        # define logging
        mylogs = logging.getLogger(__name__)
        file = logging.FileHandler(os.path.join(parameters['output_dir'], "akis_toplama.log"))
        fileformat = logging.Formatter("%(asctime)s:%(levelname)s:%(message)s",datefmt="%H:%M:%S")
        file.setFormatter(fileformat)
        mylogs.addHandler(file)
        mylogs.setLevel(logging.INFO)
        mylogs.info("\n  fd_abs_path:{} \n  fa_dem_abs_path:{}".format(
            fd_abs_path, fa_dem_abs_path))

        # TauDEM AreaD8
        taudem = TauDEM(n_core=parameters['number_of_processors'], logs=mylogs)   
        taudem.flowacc_watershed(fd_abs_path, fa_dem_abs_path)
        
        # return results
        results['fa_dem_abs_path'] = fa_dem_abs_path
        return results
    ...
    def displayName(self):
        return 'TauDEM Akış Toplama Aracı'
    ...
    def shortHelpString(self):
        html_string = '''
			...
			<h3>Akış Toplanma Haritası</h3>
			Hücrelerde tamsayı değerleri vardır. Bu değerler her hücrenin kendisi artı kendisine drene olan yukarı yamaç komşularının toplam sayısı olarak hesaplanır. 
			<!--.........................................-->
        '''
        return html_string
